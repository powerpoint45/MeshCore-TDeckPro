...
void EnvironmentSensorManager::initBasicGPS() {
    ...
    MESH_DEBUG_PRINTLN("GPS forced detected/active for T-Deck Pro");
}
...
void EnvironmentSensorManager::start_gps() {
  ...
  _location->begin();
  _location->reset();
  Serial.println("ENVSENSOR ---GPS started");
  MESH_DEBUG_PRINTLN("GPS started");
}
...
void EnvironmentSensorManager::stop_gps() {
  ...
  MESH_DEBUG_PRINTLN("GPS stopped");
}
...
void EnvironmentSensorManager::loop() {
    ...
    if (millis() - last_debug > 2000) {
      ...
      #if GPS_NMEA_DEBUG
        Serial.printf(
          "GPS dbg: active=%d valid=%d sats=%ld lat=%ld lon=%ld alt=%ld\n",
          gps_active,
          _location->isValid(),
          _location->satellitesCount(),
          _location->getLatitude(),
          _location->getLongitude(),
          _location->getAltitude()
          );
        #endif
    }
    ...
    if (millis() > next_gps_update) {
      if (gps_active && _location->isValid()) {
        ...
        #if GPS_NMEA_DEBUG
        Serial.printf("GPS FIX: lat=%f lon=%f alt=%f\n",
